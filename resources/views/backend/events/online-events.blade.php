...
@section('content')
        ...
        <div id="page-wrapper">
        <div class="clearfix">
        <h1 class="page-header">All Online Events</h1>
        </div>
                <div class="row">
                    <div class="col-lg-12">
...
<!--                                            {{-- */$i=0;/* --}}-->
                                            ...
                                            <tr class="odd gradeX">
                                                ...
                                                <td class="text-center">
                                                @can('isAdmin')
                                                    <a href="{{ route('edit-online-event', $event->id) }}"Check $1><button type="button" class="btn btn-primary">Edit</button></a>
                                                @endcan

                                                @can('isArtist')
                                                    @if($event->start_date > date('Y-m-d'))
                                                        @if(!in_array($event->id, $artistEvents))
                                                        <a href="{{ route('participate-online-event', $event->id) }}"Check $1><button type="button" class="btn btn-primary">Participate</button></a>
                                                        @endif
                                                    @endif
                                                @endcan
                                                </td>
        ...
        @can('isAdmin')
            function changeStatus(id, status) {
                ...
                if (r == true) {
                    $.ajax({
                        method: "PUT",
                        url: "/admin/online/events/changeStatus/"+id,
                        data: {
                                status:status
                             },
                        dataType: "json",
