...
<!--                                            {{-- */$i=0;/* --}}-->
                                            ...
                                            <tr class="odd gradeX">
                                                <td>{{$i}}</td>
                                                <td><img src="{{ \App\Helpers\Helper::getImage($mda->image, 7) }}" width="60" height="40px"></td>
                                                <td>{{ $mda->title }}</td>
                                                <td class="center">
                                                    @if( $mda->status == 0)
                                                    <button type="button" class="btn btn-danger" onclick="changeStatus({{ $mda->id }}, 1)">Inactive</button>
                                                    @else
                                                        <button type="button" class="btn btn-success" onclick="changeStatus({{ $mda->id }}, 0)">Active</button>
                                                    @endif
                                                </td>
                                                <td class="center"><a href="{{ route('edit-media', $mda->id) }}"><button type="button" class="btn btn-primary">Edit</button></a></td>
    ...
    <script type="text/javascript">
        ...
        function changeStatus(id, status) {
            ...
            if (r == true) {
                $.ajax({
                    method: "PUT",
                    url: "/admin/media/changeStatus/"+id,
                    data: {
                            status:status
                         },
                    dataType: "json",
